...
#     Engagement Data
    bb1 = segmented_data_test(
                            ...
                            DAYS_DATA=DAYS_DATA,
                            EXCEL_NAME="Mazda_Age_Engagement",
                            SEGMENT=SEGMENT,
                            METRIC='ENGAGEMENT'
    )
#     Video Data
...
df = big_merge_all(
                            CLIENT_ACCOUNT_ID='18ce55drkgj',
                            DAYS_DATA=45,
                            SEGMENT="AGE", 
                            BIG_EXCEL_NAME="Solidarity_Age_45_Days_FINAL"
                                            
)
# df.head(100)
import appscript

def send_email(SUBJECT, BODY, SEND_TO, EXCEL_FILE):
    ...
    import os
        f = open(os.path.expanduser(
            f"~/OneDrive - Mark1 Media & Consulting/Documents - Copy/Datorama/Twitter API/{EXCEL_FILE}.xlsx")
                )
...
send_email(
            SUBJECT='Solidarity Age',
            BODY="Solidarity Age through script",
            SEND_TO=['mj@mark1.co.za'],
            EXCEL_FILE='Solidarity_Age_45_Days_FINAL'
          )
